...
class GatedFusionModule(nn.Module):
    def __init__(self, feature_channels, target_size=(16, 16)):
        ...
        # ============ 关键修改：初始化语义偏置 ============
        self._init_semantic_bias()

    def _init_semantic_bias(self):
        """给语义特征更高的初始偏置"""
        with torch.no_grad():
            # 获取最后一层线性层
            last_layer = self.gate_network[2]
            # 设置偏置：纹理偏置0，结构偏置0，语义偏置1.0
            last_layer.bias.data = torch.tensor([0.0, 0.0, 1.0])
    ...
    def forward(self, shallow_feat, mid_feat, deep_feat):
        ...
        logits = self.gate_network(concat_gap)  # 现在语义的logits初始就高
